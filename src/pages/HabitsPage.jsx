...
export default function HabitsPage({ habits, form, habitErrors, quickSettings, updateFormField, updateQuickSetting, toggleDay, handleCreateHabit, resetHabitForm, addQuickHabit, deleteHabit }) {
  return (
    <>
          ...
          {QUICK_HABITS.map((habit) => {
            ...
            return (
              <article key={habit.id} className="quick-habit-card">
                <div className="quick-habit-card__head"><strong><span className="habit-emoji" aria-hidden="true">{habit.emoji}</span>{habit.name}</strong><span>{HABIT_TYPES.find((item) => item.value === habit.type)?.label}</span></div>
      ...
      <section className="panel">
        <div className="section-header"><div><p className="section-kicker">직접 추가</p><h2>새 습관 만들기</h2></div><Plus size={18} /></div>
        <HabitForm form={form} errors={habitErrors} updateFormField={updateFormField} toggleDay={toggleDay} handleCreateHabit={handleCreateHabit} onCancel={resetHabitForm} />
      </section>
